...
>((props, forwardedRef) => {
  ...
  const _renderShape = React.useCallback(
    (
      ...
      const getShapePath = (shapeType: LegendShape, x: number, y: number, w: number): string => {
        const allPointPaths = [
          // circle path (index 0)
          `M${x - w / 2} ${y} A${w / 2} ${w / 2} 0 1 0 ${x + w / 2} ${y} M${x - w / 2} ${y} A ${w / 2} ${w / 2} 0 1 1 ${
            x + w / 2
          } ${y}`,
          // square (index 1)
          `M${x - w / 2} ${y - w / 2} L${x + w / 2} ${y - w / 2} L${x + w / 2} ${y + w / 2} L${x - w / 2} ${
            y + w / 2
          } Z`,
          // triangle (index 2)
          `M${x - w / 2} ${y - 0.2886 * w} H ${x + w / 2} L${x} ${y + 0.5774 * w} Z`,
          // diamond (index 3)
          `M${x} ${y - w / 2} L${x + w / 2} ${y} L${x} ${y + w / 2} L${x - w / 2} ${y} Z`,
          // pyramid (index 4)
          `M${x} ${y - 0.5774 * w} L${x + w / 2} ${y + 0.2886 * w} L${x - w / 2} ${y + 0.2886 * w} Z`,
          // hexagon (index 5)
          `M${x - 0.5 * w} ${y - 0.866 * w} L${x + 0.5 * w} ${y - 0.866 * w} L${x + w} ${y} L${x + 0.5 * w} ${
            y + 0.866 * w
          } L${x - 0.5 * w} ${y + 0.866 * w} L${x - w} ${y} Z`,
          // pentagon (index 6)
          `M${x} ${y - 0.851 * w} L${x + 0.6884 * w} ${y - 0.2633 * w} L${x + 0.5001 * w} ${y + 0.6884 * w} L${
            x - 0.5001 * w
          } ${y + 0.6884 * w} L${x - 0.6884 * w} ${y - 0.2633 * w} Z`,
          // octagon (index 7)
          `M${x - 0.5001 * w} ${y - 1.207 * w} L${x + 0.5001 * w} ${y - 1.207 * w} L${x + 1.207 * w} ${
            y - 0.5001 * w
          } L${x + 1.207 * w} ${y + 0.5001 * w} L${x + 0.5001 * w} ${y + 1.207 * w} L${x - 0.5001 * w} ${
            y + 1.207 * w
          } L${x - 1.207 * w} ${y + 0.5001 * w} L${x - 1.207 * w} ${y - 0.5001 * w} Z`,
          // cross (index 8)
          `M${x - w / 2} ${y - w / 2} L${x + w / 2} ${y - w / 2} L${x + w / 2} ${y - w / 6} L${x + w / 6} ${
            y - w / 6
          } L${x + w / 6} ${y + w / 6} L${x + w / 2} ${y + w / 6} L${x + w / 2} ${y + w / 2} L${x - w / 2} ${
            y + w / 2
          } L${x - w / 2} ${y + w / 6} L${x - w / 6} ${y + w / 6} L${x - w / 6} ${y - w / 6} L${x - w / 2} ${
            y - w / 6
          } Z`,
          // x (index 9)
          `M${x - w / 2} ${y - w / 2} L${x - w / 6} ${y - w / 6} L${x + w / 6} ${y + w / 6} L${x + w / 2} ${
            y + w / 2
          } L${x + w / 6} ${y + w / 6} L${x - w / 6} ${y - w / 6} Z M${x + w / 2} ${y - w / 2} L${x + w / 6} ${
            y - w / 6
          } L${x - w / 6} ${y + w / 6} L${x - w / 2} ${y + w / 2} L${x - w / 6} ${y + w / 6} L${x + w / 6} ${
            y - w / 6
          } Z`,
          // rectangle (index 10)
          `M${x - w / 2} ${y - w / 3} L${x + w / 2} ${y - w / 3} L${x + w / 2} ${y + w / 3} L${x - w / 2} ${
            y + w / 3
          } Z`,
        ];

        const shapeIndexMap: { [key: string]: number } = {
          circle: 0,
          square: 1,
          triangle: 2,
          diamond: 3,
        ...
        const index = shapeIndexMap[shapeType] || 0;
        return allPointPaths[index];
      };

      const pathData = getShapePath(shape, 0, 0, size * 2);
    ...
    (xElement: SVGElement, containerHeight: number): JSX.Element[] => {
      ...
      for (let i = _points.current?.length - 1; i >= 0; i--) {
        ...
        for (let j = 0; j < _points.current?.[i]?.data?.length; j++) {
          ...
          const pointShape = (_points.current?.[i]?.data[j] as IScatterChartDataPoint).shape;
          ...
          // Determine the correct x value to use for the scale
          let xForScaleFixed;
          if (_xAxisType === XAxisTypes.NumericAxis) {
            xForScaleFixed = typeof x === 'number' ? x : 0;
          } else if (_xAxisType === XAxisTypes.DateAxis) {
            xForScaleFixed = x instanceof Date ? x : new Date(x);
          } else {
            xForScaleFixed = x; // string axis
          }

          // Ensure yForScale is always a number
          let yForScaleFixed = typeof y === 'number' ? y : Number(y);

          // Only highlight points for the hovered or selected legend
            ...
            <>
              ...
              {text && (
                <text
                  ...
                  x={(() => {
                    if (_xAxisType === XAxisTypes.NumericAxis) {
                      return _xAxisScale.current?.(typeof x === 'number' ? x : 0) + _xBandwidth.current;
                    } else if (_xAxisType === XAxisTypes.DateAxis) {
                      return _xAxisScale.current?.(x instanceof Date ? x : new Date(x)) + _xBandwidth.current;
                    } else {
                      return _xAxisScale.current?.(x) + _xBandwidth.current;
                    }
                  })()}
                  y={_yAxisScale.current?.(yForScaleFixed) + Math.max(circleRadius + 12, 16)}
